...
window.addEventListener('load', function () {
    setTimeout(function () {
        ...
        if (loader) {
            ...
            // Start hero star generation after load
            generateHeroStars();
            // Start floating hearts after load
            startFloatingHearts();
            // Start scroll reveal observer
            initScrollReveal();
            // Start typewriter when letter section comes into view
            initTypewriterObserver();
        }
    }, 2200); // Match loading bar duration
});
...
function startTypewriter() {
    ...
    function type() {
        if (charIndex < fullText.length) {
            ...
            cursor.remove();
            if (seal) {
                setTimeout(function () { seal.classList.add('show'); }, 400);
            }
        }
    }
...
function changePhoto(direction) {
    if (typeof galleryData === 'undefined') return;
    currentPhoto = (currentPhoto + direction + galleryData.length) % galleryData.length;
    updateModal();
}
...
function closeModal(event) {
    ...
    if (!event || event.target === modal || event.currentTarget.classList.contains('modal-close')) {
...
function showNextReason() {
    ...
    setTimeout(function () {
        ...
        if (counter) counter.textContent = reasonIndex === 0 ? reasons.length : reasonIndex;
...
const confettiColors = [
    '#FF6B9D', '#FFD6E5', '#F7C873', '#FF8FB5',
    '#FFFFFF', '#E8547A', '#FFB3CE', '#FFF5F8',
    '#FF4081', '#FFCDD2'
];
    ...
    galleryImgs.forEach(function (img, i) {
        img.addEventListener('error', function () {
            ...
            if (wrapper) {
                ...
                if (inner) {
                    inner.style.cssText = 'display:flex; flex-direction:column; align-items:center; gap:0.4rem;';
                }
                ...
                if (phLabel) {
                    phLabel.style.cssText = 'font-size:0.78rem; font-weight:600; color:#FF6B9D;';
                }
                const phHint = wrapper.querySelector('.ph-hint');
                if (phHint) {
                    phHint.style.cssText = 'font-size:0.65rem; color:#A07080; font-family: monospace;';
                }
            }
        });
    });
